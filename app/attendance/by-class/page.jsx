...
//                   {lecture.subId.subname} ({lecture.subId.subcode}) - Year {lecture.year} Section {lecture.section} - {lecture.facId.facName}
//                 </option>
//               ))}
//             </select>
...
//                 <tr>
//                   <th className="px-6 py-3 border-b text-left text-xs font-medium text-gray-500 uppercase">Student ID</th>
...
//                     <td className="px-6 py-4 text-sm text-gray-900">{att.studentId}</td>
      ...
      <div className="max-w-6xl mx-auto bg-white p-6 rounded-xl shadow-md">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          ...
          <div className="space-y-4">
            ...
            >
              {/* ✅ fixed key */}
              <option key="default" value="">Select Lecture</option>

              {lectures
                .filter((lecture) => lecture?._id) // ✅ avoid bad data
                .map((lecture, index) => (
                  <option
                    key={lecture._id || `lecture-${index}`} // ✅ unique key
                    value={lecture._id}
                  >
                    {/* ✅ optional chaining fix */}
                    {lecture.subId?.subname || "N/A"} (
                    {lecture.subId?.subcode || "N/A"}) - Year{" "}
                    {lecture.year || "N/A"} Section{" "}
                    {lecture.section || "N/A"} -{" "}
                    {lecture.facId?.facName || "N/A"}
                  </option>
                ))}
            </select>
          ...
          <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
            ...
            <p>
              ...
              {faculty?.facName || "N/A"} ({faculty?.facEmail || "-"})
            </p>
            <p>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {attendance.map((att, idx) => (
                  <tr key={att.studentId || idx}>
                    <td className="px-6 py-4 text-sm">{att.studentId}</td>
                    <td className="px-6 py-4 text-sm font-medium">{att.studentName}</td>
        ...
        {!loading && selectedLecture && attendance.length === 0 && (
